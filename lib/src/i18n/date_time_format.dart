...
class DateTimeFormat {
  ...
  static final String RFC2822_PATTERN = "EEE, d MMM yyyy HH:mm:ss Z";
  static final String ISO8601_PATTERN = "yyyy-MM-dd'T'HH:mm:ss.SSSZZZ";

  static final int NUMBER_BASE = 10;
  static final int JS_START_YEAR = 1900;

  static final Map<String, DateTimeFormat> cache = new Map<String, DateTimeFormat>();

  static final int NUM_MILLISECONDS_IN_DAY = 24 * 60 * 60000;

  static final String PATTERN_CHARS = "GyMLdkHmsSEcDahKzZv";

  // Note: M & L must be the first two characters
  static final String NUMERIC_FORMAT_CHARS = "MLydhHmsSDkK";
  ...
  static DateTimeFormat getPredefinedFormat(PredefinedFormat predef) {
    if (usesFixedEnglishStrings(predef)) {
      String pattern;
      switch (predef) {
        case PredefinedFormat.RFC_2822:
          pattern = RFC2822_PATTERN;
          break;
        case PredefinedFormat.ISO_8601:
          pattern = ISO8601_PATTERN;
          break;
        default:
          throw new Exception("Unexpected predef type $predef");
      }
    ...
    String pattern;
    switch (predef) {
      case PredefinedFormat.DATE_FULL:
        pattern = dtfi.dateFormatFull();
        break;
      case PredefinedFormat.DATE_LONG:
        pattern = dtfi.dateFormatLong();
        break;
      case PredefinedFormat.DATE_MEDIUM:
        pattern = dtfi.dateFormatMedium();
        break;
      case PredefinedFormat.DATE_SHORT:
        pattern = dtfi.dateFormatShort();
        break;
      case PredefinedFormat.DATE_TIME_FULL:
        pattern = dtfi.dateTimeFull(dtfi.timeFormatFull(),
            dtfi.dateFormatFull());
        break;
      case PredefinedFormat.DATE_TIME_LONG:
        pattern = dtfi.dateTimeLong(dtfi.timeFormatLong(),
            dtfi.dateFormatLong());
        break;
      case PredefinedFormat.DATE_TIME_MEDIUM:
        pattern = dtfi.dateTimeMedium(dtfi.timeFormatMedium(),
            dtfi.dateFormatMedium());
        break;
      case PredefinedFormat.DATE_TIME_SHORT:
        pattern = dtfi.dateTimeShort(dtfi.timeFormatShort(),
            dtfi.dateFormatShort());
        break;
      case PredefinedFormat.DAY:
        pattern = dtfi.formatDay();
        break;
      case PredefinedFormat.HOUR24_MINUTE:
        pattern = dtfi.formatHour24Minute();
        break;
      case PredefinedFormat.HOUR24_MINUTE_SECOND:
        pattern = dtfi.formatHour24MinuteSecond();
        break;
      case PredefinedFormat.HOUR_MINUTE:
        pattern = dtfi.formatHour12Minute();
        break;
      case PredefinedFormat.HOUR_MINUTE_SECOND:
        pattern = dtfi.formatHour12MinuteSecond();
        break;
      case PredefinedFormat.MINUTE_SECOND:
        pattern = dtfi.formatMinuteSecond();
        break;
      case PredefinedFormat.MONTH:
        pattern = dtfi.formatMonthFull();
        break;
      case PredefinedFormat.MONTH_ABBR:
        pattern = dtfi.formatMonthAbbrev();
        break;
      case PredefinedFormat.MONTH_ABBR_DAY:
        pattern = dtfi.formatMonthAbbrevDay();
        break;
      case PredefinedFormat.MONTH_DAY:
        pattern = dtfi.formatMonthFullDay();
        break;
      case PredefinedFormat.MONTH_NUM_DAY:
        pattern = dtfi.formatMonthNumDay();
        break;
      case PredefinedFormat.MONTH_WEEKDAY_DAY:
        pattern = dtfi.formatMonthFullWeekdayDay();
        break;
      case PredefinedFormat.TIME_FULL:
        pattern = dtfi.timeFormatFull();
        break;
      case PredefinedFormat.TIME_LONG:
        pattern = dtfi.timeFormatLong();
        break;
      case PredefinedFormat.TIME_MEDIUM:
        pattern = dtfi.timeFormatMedium();
        break;
      case PredefinedFormat.TIME_SHORT:
        pattern = dtfi.timeFormatShort();
        break;
      case PredefinedFormat.YEAR:
        pattern = dtfi.formatYear();
        break;
      case PredefinedFormat.YEAR_MONTH:
        pattern = dtfi.formatYearMonthFull();
        break;
      case PredefinedFormat.YEAR_MONTH_ABBR:
        pattern = dtfi.formatYearMonthAbbrev();
        break;
      case PredefinedFormat.YEAR_MONTH_ABBR_DAY:
        pattern = dtfi.formatYearMonthAbbrevDay();
        break;
      case PredefinedFormat.YEAR_MONTH_DAY:
        pattern = dtfi.formatYearMonthFullDay();
        break;
      case PredefinedFormat.YEAR_MONTH_NUM:
        pattern = dtfi.formatYearMonthNum();
        break;
      case PredefinedFormat.YEAR_MONTH_NUM_DAY:
        pattern = dtfi.formatYearMonthNumDay();
        break;
      case PredefinedFormat.YEAR_MONTH_WEEKDAY_DAY:
        pattern = dtfi.formatYearMonthWeekdayDay();
        break;
      case PredefinedFormat.YEAR_QUARTER:
        pattern = dtfi.formatYearQuarterFull();
        break;
      case PredefinedFormat.YEAR_QUARTER_ABBR:
        pattern = dtfi.formatYearQuarterShort();
        break;
      default:
        throw new Exception("Unexpected predefined format ${predef}");
    }
   ...
   */
  static bool usesFixedEnglishStrings(PredefinedFormat predef) {
    switch (predef) {
      case PredefinedFormat.RFC_2822:
        return true;
      case PredefinedFormat.ISO_8601:
  ...
  static DateTimeFormatInfo getDefaultDateTimeFormatInfo() {
    // MUSTFIX(jat): implement
    return LocaleInfo.getCurrentLocale().getDateTimeFormatInfo();
  }
  
  final List<PatternPart> patternParts = new List<PatternPart>();
   ...
   */
  DateTimeFormat(this.pattern, [DateTimeFormatInfo dtfi = null]) {
    if (dtfi == null) {
      this.dateTimeFormatInfo = getDefaultDateTimeFormatInfo();
    } else {
  ...
  String format(Date date, [TimeZone timeZone = null]) {
    ...
    if (timeZone == null) {
      timeZone = createTimeZone(getTimezoneOffset(date));
    }
    int diff = (getTimezoneOffset(date) - timeZone.getOffset(date)) * 60000;
    Date keepDate = new Date(date.millisecondsSinceEpoch + diff);
    Date keepTime = keepDate;
    if (getTimezoneOffset(keepDate) != getTimezoneOffset(date)) {
      if (diff > 0) {
        diff -= NUM_MILLISECONDS_IN_DAY;
      } else {
        diff += NUM_MILLISECONDS_IN_DAY;
      }
      keepTime = new Date(date.millisecondsSinceEpoch + diff);
    }
      ...
      if ((ch.compareTo('a') >= 0 && ch.compareTo('z') <= 0) || (ch.compareTo('A') >= 0 && ch.compareTo('Z') <= 0)) {
        ...
        while (!trailQuote) {
          ...
          if (j >= n) {
            ...
            throw new Exception("Missing trailing \'");
          }
          ...
          toAppendTo.add(pattern.substring(i, j));
          i = j + 1;
        }
      } else {
   ...
   */
  void format0To11Hours(StringBuffer buf, int count, Date date) {
    int value = date.hour % 12;
   ...
   * @param date hold the date object to be formatted
   */
  void format0To23Hours(StringBuffer buf, int count, Date date) {
    int value = date.hour;
    zeroPaddingNumber(buf, value, count);
  }
   ...
   * @param date hold the date object to be formatted
   */
  void format1To12Hours(StringBuffer buf, int count, Date date) {
    int value = date.hour % 12;
    if (value == 0) {
   ...
   * @param date hold the date object to be formatted
   */
  void format24Hours(StringBuffer buf, int count, Date date) {
    int value = date.hour;
    if (value == 0) {
   ...
   * @param date hold the date object to be formatted
   */
  void formatAmPm(StringBuffer buf, Date date) {
    if (date.hour >= 12 && date.hour < 24) {
      buf.add(dateTimeFormatInfo.ampms()[1]);
    } else {
      buf.add(dateTimeFormatInfo.ampms()[0]);
    }
  }
   ...
   * @param date hold the date object to be formatted
   */
  void formatDate(StringBuffer buf, int count, Date date) {
    int value = date.day;
    zeroPaddingNumber(buf, value, count);
  }
   ...
   * @param date hold the date object to be formatted
   */
  void formatDayOfWeek(StringBuffer buf, int count, Date date) {
    int value = date.day;
    if (count == 5) {
      buf.add(dateTimeFormatInfo.weekdaysNarrow()[value]);
    } else if (count == 4) {
      buf.add(dateTimeFormatInfo.weekdaysFull()[value]);
    } else {
      buf.add(dateTimeFormatInfo.weekdaysShort()[value]);
    }
  }
   ...
   * @param date hold the date object to be formatted
   */
  void formatEra(StringBuffer buf, int count, Date date) {
    int value = date.year >= -JS_START_YEAR ? 1 : 0;
    if (count >= 4) {
      buf.add(dateTimeFormatInfo.erasFull()[value]);
    } else {
      buf.add(dateTimeFormatInfo.erasShort()[value]);
    }
  }
   ...
   * @param date hold the date object to be formatted
   */
  void formatFractionalSeconds(StringBuffer buf, int count, Date date) {
    /*
    ...
    if (count == 1) {
      value = dart_math.min((value + 50) / 100, 9); // Round to 100ms, clamp to 9
      buf.add(('0${value}'));
    } else if (count == 2) {
      value = dart_math.min((value + 5) / 10, 99); // Round to 10ms, clamp to 99
   ...
   * @param date hold the date object to be formatted
   */
  void formatMinutes(StringBuffer buf, int count, Date date) {
    int value = date.minute;
    zeroPaddingNumber(buf, value, count);
  }
   ...
   * @param date hold the date object to be formatted
   */
  void formatMonth(StringBuffer buf, int count, Date date) {
    int value = date.month;
    switch (count) {
      case 5:
        buf.add(dateTimeFormatInfo.monthsNarrow()[value]);
        break;
      case 4:
        buf.add(dateTimeFormatInfo.monthsFull()[value]);
        break;
      case 3:
        buf.add(dateTimeFormatInfo.monthsShort()[value]);
        break;
      default:
   ...
   * @param date hold the date object to be formatted
   */
  void formatQuarter(StringBuffer buf, int count, Date date) {
    int value = date.month ~/ 3;
    if (count < 4) {
      buf.add(dateTimeFormatInfo.quartersShort()[value]);
    } else {
      buf.add(dateTimeFormatInfo.quartersFull()[value]);
    }
  }
   ...
   * @param date hold the date object to be formatted
   */
  void formatSeconds(StringBuffer buf, int count, Date date) {
    int value = date.second;
    zeroPaddingNumber(buf, value, count);
  }
   ...
   * @param date hold the date object to be formatted
   */
  void formatStandaloneDay(StringBuffer buf, int count, Date date) {
    int value = date.day;
    if (count == 5) {
      buf.add(dateTimeFormatInfo.weekdaysNarrowStandalone()[value]);
    } else if (count == 4) {
      buf.add(dateTimeFormatInfo.weekdaysFullStandalone()[value]);
    } else if (count == 3) {
      buf.add(dateTimeFormatInfo.weekdaysShortStandalone()[value]);
    } else {
   ...
   * @param date hold the date object to be formatted
   */
  void formatStandaloneMonth(StringBuffer buf, int count, Date date) {
    int value = date.month;
    if (count == 5) {
      buf.add(dateTimeFormatInfo.monthsNarrowStandalone()[value]);
    } else if (count == 4) {
      buf.add(dateTimeFormatInfo.monthsFullStandalone()[value]);
    } else if (count == 3) {
      buf.add(dateTimeFormatInfo.monthsShortStandalone()[value]);
    } else {
      zeroPaddingNumber(buf, value + 1, count);
    }
  }
   ...
   * @param date hold the date object to be formatted
   */
  void formatTimeZone(StringBuffer buf, int count, Date date,
      TimeZone timeZone) {
    if (count < 4) {
      buf.add(timeZone.getShortName(date));
    } else {
      buf.add(timeZone.getLongName(date));
    }
  }
   ...
   * @param date hold the date object to be formatted
   */
  void formatTimeZoneRFC(StringBuffer buf, int count, Date date,
      TimeZone timeZone) {
    if (count < 3) {
      buf.add(timeZone.getRFCTimeZoneString(date));
    } else if (count == 3) {
      buf.add(timeZone.getISOTimeZoneString(date));
    } else {
      buf.add(timeZone.getGMTString(date));
    }
  }
   ...
   * @param date hold the date object to be formatted
   */
  void formatYear(StringBuffer buf, int count, Date date) {
    int value = date.year + JS_START_YEAR;
    if (value < 0) {
      value = -value;
    }
    switch (count) {
   ...
   * @return repeat count
   */
  int getNextCharCountInPattern(String pattern, int start) {
    String ch = pattern[start];
    int next = start + 1;
    while (next < pattern.length && pattern[next] == ch) {
      next++;
    }
    return next - start;
  }
   ...
   * others.
   */
  void identifyAbutStart() {
    // 'abut' parts are continuous numeric parts. abutStart is the switch
    // point from non-abut to abut.
    bool abut = false;

    int len = patternParts.length;
    for (int i = 0; i < len; i++) {
      if (isNumeric(patternParts[i])) {
        // If next part is not following abut sequence, and isNumeric.
        if (!abut && i + 1 < len && isNumeric(patternParts[i + 1])) {
          abut = true;
          patternParts[i].abutStart = true;
        }
      } else {
        abut = false;
      }
   ...
   */
  bool isNumeric(PatternPart part) {
    if (part.count <= 0) {
      return false;
    }
    int i = NUMERIC_FORMAT_CHARS.indexOf(part.text[0]);
   ...
   */
  int matchString(String text, int start, List<String> data, List<int> pos) {
   ...
   * {@link Exception} is thrown if either the text is empty or
   ...
   * @throws Exception if the entire text could not be converted
   *           into a number
   */
  Date parse(String text, bool strict) {
    Date curDate = new Date.now();
    Date date = new Date(curDate.year, curDate.month, curDate.day);
    int charsConsumed = parseDate(text, 0, date, strict);
   ...
   */
  int parseDate(String text, int start, Date date, bool strict) {
    DateRecord cal = new DateRecord();
    List<int> parsePos = [start];
    ...
    for (int i = 0; i < patternParts.length; ++i) {
      PatternPart part = patternParts[i];
...
//        } else if (text.startsWith(part.text, parsePos[0])) {
        } else if (text.startsWith(part.text)) {
   ...
   */
  int parseInt(String text, List<int> pos) {
    int ret = 0;
    int ind = pos[0];
    if (ind >= text.length) {
      return -1;
    ...
    String ch = text[ind];
    while (ch.compareTo('0') >= 0 && ch.compareTo('9') <= 0) {
      ret = ret * 10 + int.parse(ch);
      ind++;
      if (ind >= text.length) {
        break;
      }
   ...
   *         <code>false</code>
   */
  bool parseTimeZoneOffset(String text, List<int> pos, DateRecord cal) {
   ...
   */
  void skipSpace(String text, List<int> pos) {
    while (pos[0] < text.length && WHITE_SPACE.indexOf(text[pos[0]]) >= 0) {
      pos[0]++;
    }
  }
   ...
   * 
   */
  bool subFormat(StringBuffer buf, String ch, int count, Date date,
   ...
   */
  bool subParse(String text, List<int> pos, PatternPart part, int digitCount, DateRecord cal) {

    skipSpace(text, pos);

    int start = pos[0];
    String ch = part.text[0];
    ...
    switch (ch) {
      ...
      case 'Z': // time zone RFC
        ...
        if (start < text.length && text[start] == 'Z') {
          pos[0]++;
          cal.setTzOffset(0);
          return true;
        }
   ...
   *         <code>false</code>
   */
  bool subParseDayOfWeek(String text, List<int>pos, int start, DateRecord cal) {
    int value;
    // 'E' - DAY_OF_WEEK
   ...
   *         <code>false</code>
   */
  bool subParseFractionalSeconds(int value, int start, int end, DateRecord cal) {
   ...
   * @return <code>true</code> if parsing successful
   */
  bool subParseMonth(String text, List<int> pos, DateRecord cal, int value, int start) {
   ...
   *         <code>false</code>
   */
  bool subParseStandaloneDay(String text, List<int> pos, int start, DateRecord cal) {
    int value;
    // 'c' - DAY_OF_WEEK
    // Want to be able to parse both short and long forms.
   ...
   * @return <code>true</code> if parsing successful
   */
  bool subParseStandaloneMonth(String text, List<int> pos, DateRecord cal, int value, int start) {
   ...
   *         <code>false</code>
   */
  bool subParseTimeZoneInGMT(String text, int start, List<int> pos, DateRecord cal) {
   ...
   */
  bool subParseYear(String text, List<int> pos, int start, int value, PatternPart part, DateRecord cal) {
    String ch = ' ';
    if (value < 0) {
      if (pos[0] >= text.length) {
        return false;
    ...
    if (ch == ' ' && (pos[0] - start) == 2 && part.count == 2) {
      ...
      int defaultCenturyStartYear = date.year + 1900 - 80;
   ...
   *          to reach this width
   */
  void zeroPaddingNumber(StringBuffer buf, int value, int minWidth) {
    int b = NUMBER_BASE;
...
class PatternPart {
  String text;
  int count; // 0 has a special meaning, it stands for literal
  bool abutStart = false;

  PatternPart(this.text, this.count);
}